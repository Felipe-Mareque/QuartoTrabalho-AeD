...
// supondo que terei o vetor ordenado
	...
	for ( x; x< numsSize; x++){
		int soma;
		soma = nums[k] + nums[x]+ nums[y] + nums[z];

		if (soma == target){
		// tenho que colocar no vetor tambem 
		...
		// na minha cabeça eu tenho que evitar de alguma forma que um passe o outro
	}
