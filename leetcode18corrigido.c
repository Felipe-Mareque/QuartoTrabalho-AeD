...
void merge( int v[], int inicio, int fim ) {
	...
	while ( esquerda <= meio && direita <= fim ) {
		...
		if ( v[esquerda] <= v[direita] ) {
			auxiliar[i] = v[esquerda];
			esquerda++;
		}
		else {
			auxiliar[i] = v[direita];
			direita++;
		}
...
int **fourSum( int *nums, int numsSize, int target,
	...
	capacidade = numsSize * numsSize;
	resultados = malloc( capacidade * sizeof( int * ) );

	inicio = 0;
	fim = numsSize - 1;

	mergeSort( nums, inicio, fim );

	/* supondo que terei o vetor ordenado */
	for ( int k = 0; k < numsSize - 3; k++ ) {
		...
		for ( ; x < numsSize - 2; x++ ) {
			...
			while ( y < z ) {
				...
				soma = ( long long )nums[k] + nums[x] + nums[y] + nums[z];

				if ( soma == target ) {

					resultados[linha] = malloc( 4 * sizeof( int ) );

					resultados[linha][0] = nums[k];
					resultados[linha][1] = nums[x];
					resultados[linha][2] = nums[y];
					resultados[linha][3] = nums[z];
	...
	*returnSize = quantidade;

	*returnColumnSizes = malloc( quantidade * sizeof( int ) );
